...
unsigned int Graph::theoreticalMergeVertices(unsigned int adjList1, unsigned int adjList2) {
    ...
    while (!(targetIt == adjLists[target - 1]->end() && delIt == adjLists[del - 1]->end())) {
        if (abs(*targetIt) == abs(*delIt) && targetIt != adjLists[target - 1]->end() &&
            delIt != adjLists[del - 1]->end()) {
            // in both
            ...
            delIt++;
            targetIt++;
        } else if (abs(*targetIt) > abs(*delIt) && delIt != adjLists[del - 1]->end() ||
                   targetIt == adjLists[target - 1]->end()) {
            // only in del
            if (abs(*delIt) != target) {
                if (*delIt > 0) {
                    // del is positive
...
unsigned int Graph::realMergeVertices(unsigned int adjList1, unsigned int adjList2) {
    ...
    while (targetIt != adjLists[target - 1]->end() || delIt != adjLists[del - 1]->end()) {
        ...
        if (edgeValue(*targetIt) == edgeValue(*delIt) && targetIt != adjLists[target - 1]->end() &&
            delIt != adjLists[del - 1]->end()) {
            ...
            delIt++;
            targetIt++;
        } else if (edgeValue(*targetIt) > edgeValue(*delIt) && delIt != adjLists[del - 1]->end() ||
                   targetIt == adjLists[target - 1]->end()) {
            // only in del
            if (abs(*delIt) != target) {
...
void Graph::updateMinMerges(vector<tuple<int, int>> *minMerges, unsigned int vertex1, list<int> delAdjList,
                            unsigned int delVertex) {
