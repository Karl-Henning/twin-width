...
a co-graph. Introduced in 2020, the research about this parameter lead to many theoretical results but as of now it is not real practical tested. \todo{https://ieeexplore.ieee.org/document/9317878 + quellen für results einbinden} So our goal is to implement and test it to bridge the gap between theory and practice. \\ 
The twin-width algorithm is a new approach to solve graph theory problems that are commonly solved by the tree-width algorithm. The starting point of our paper will be a short summary of the algorithm. Afterwards we will introduce a way to implement it in C++. \\%add reference to https://pacechallenge.org/2023/
%Beschreibung...

Twin-with is a non-negative integer measuring a graph's distance to being a co-graph. A
co-graph is described by the following properties: \todo{axioms?}
...
Now\todo{weg} the idea is that a graph has bounded twin-width if we can contract it to a singleton
by contracting near-twins together (two vertices whose neighborhoods differ only on a
bounded number of elements). We actually\todo{weg} track the errors and call the edges we need to
add "red edges".\\
A graph has now\todo{weg} twin-width $d$ if we can bound the degree in red edges by the treshhold $d$.
Because co-graphs don't produce errors they always have twin-width 0.\todo{Satzstellung drehen} \\
%Add grapic example

graphs that can be reduced to a single vertex by a process of repeatedly finding any two
twin vertices and merging them into a single vertex. \todo{Satz unvollständig} The definition of twin-width mimics
...
edges that differ will be marked red {A / B \& B/A} == red. \todo{Formel (?) hier komisch}\\
